...
class EmployeeYearlyPayrollSalary extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_yearlypayroll_view') == 0 || $this->session->userdata('right_yearlypayroll_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['title'] = 'JCORE - Employee Yearly Payroll Salary';
        $this->load->view('employee_yearly_payroll_salary_view', $data);
    }
