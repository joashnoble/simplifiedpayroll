...
class DailyTimeRecord_model extends CORE_Model {
    ...
    function get_dtr_list($company_id,$pay_period_id,$ref_branch_id,$ref_department_id){
        $query = $this->db->query("SELECT 
                        ...
                        COALESCE((SELECT SUM(deduction_amount) FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 1),0) as sss_loan,

                        COALESCE((SELECT loan_list.loan_id FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 1),0) as sss_loan_id,

                        COALESCE((SELECT SUM(deduction_amount) FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 2),0) as pagibig_loan,

                        COALESCE((SELECT loan_list.loan_id FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 2),0) as pagibig_loan_id,

                        COALESCE((SELECT SUM(deduction_amount) FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 3),0) as cash_advance,

                        COALESCE((SELECT loan_list.loan_id FROM pay_slip_deduction
                        LEFT JOIN loan_list ON loan_list.loan_id = pay_slip_deduction.deduction_id
                        WHERE pay_slip_deduction.pay_slip_id = ps.pay_slip_id
                        AND loan_list.ref_loan_id = 3),0) as cash_advance_id
    ...
    function get_refresh_dtr_list($company_id,$count,$frequency,$pay_period_id,$ref_branch_id,$ref_department_id,$sequence,$pagibig1=0){
        $query = $this->db->query("SELECT 
                        dtr.no_of_hrs,
                        dtr.overtime,
                        dtr.holiday_pay,
