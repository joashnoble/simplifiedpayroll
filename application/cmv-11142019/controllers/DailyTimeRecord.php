    ...
    function transaction($txn = null) {
        switch ($txn) {
            ...
            case 'list':
                ...
                if ($pay_period_id == "" || $pay_period_id == null){
                    ...
                    if (count($chck_dtr_status) > 0){
                        $response['data'] = $this->DailyTimeRecord_model->get_dtr_list($this->session->company_id,$pay_period_id,$ref_branch_id,$ref_department_id);
                    }else{
            ...
            case 'refresh_list':
                ...
                if (count($chck_dtr_status) > 0){
                    $response['data'] = $this->DailyTimeRecord_model->get_refresh_dtr_list($this->session->company_id,$count,$frequency,$pay_period_id,$ref_branch_id,$ref_department_id,$sequence);
                }else{
                    $response['data'] = $this->Employee_model->get_emp_list($this->session->company_id,$count,$frequency,$ref_branch_id,$ref_department_id,$sequence);
                }
                echo json_encode($response);
