...
class EmployeePayrollHistory extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_payrollhistory_view') == 0 || $this->session->userdata('right_payrollhistory_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['title'] = 'JCORE - Employee Payroll History';
        $this->load->view('employee_payroll_history_view', $data);
    }
