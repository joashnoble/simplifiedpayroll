                                ...
                                <div class="panel-inside">
                                    <div class="panel-inside1">
                                            <div class="panel-heading emp-panel-header">
                                                <span id="emp-tbl-title">Section List</span>
                                                <button class="btn btn-default right_section_create"  id="btn_new" style="float: right;margin-top: 8px;" title="Create New Section" >
                                                <i class="fa fa-plus"></i> New
                                                </button>
                                            </div>
                                            <div class="panel-body table-responsive" style="padding-top:8px;">
                                                <table id="tbl_section" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                                    <thead>
                                                        <tr>
                                                            <th>Section Name</th>
                                                            <th>Description</th>
            ...
            <div class="modal-body">
                <center>
                    <p id="modal-body-message" style="font-size: 12pt!important;font-family: calibri;">
                            Remove <sectionname id="sectionname"></sectionname> Section?
                    </p>
                </center>
            </div>
...
<div id="modal_create_section" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static"><!--modal-->
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
                <h4 class="modal-title"><span id="modal_mode"> </span><i class="fa fa-list"></i> Section : <transaction class="transaction_type"></transaction></h4>
            </div>
            <div class="modal-body">
                <form id="frm_section">
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group" style="margin-bottom:0px;">
                        <label class="boldlabel"><span class="label-required">*</span> Section Name :</label>
                        <input type="text" class="form-control" id="section" name="section" placeholder="Section Name" data-error-msg="Section Name is Required!" required>
                    </div>
                  </div>
                </div><br>
                <div class="row">
...
$(document).ready(function(){
    ...
    var initializeControls=function(){
        dt=$('#tbl_section').DataTable({
            "aLengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
            "ajax" : "RefSection/transaction/list",
            "columns": [
                { targets:[0],data: "section" },
                { targets:[1],data: "description" },
                {
                    targets:[2],
                    render: function (data, type, full, meta){

                        return '<center>'+right_section_edit+right_section_delete+'</center>';
                    }
                }
    ...
    var bindEventHandlers=(function(){
        ...
        $('#tbl_section tbody').on( 'click', 'tr td.details-control', function () {
        ...
        $('#tbl_section tbody').on('click','button[name="edit_info"]',function(){
            _txnMode="edit";
            $('.transaction_type').text('Edit');
            $('#modal_create_section').modal('show');
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.ref_section_id;
        ...
        $('#tbl_section tbody').on('click','button[name="remove_info"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.ref_section_id;
            _sectionname=data.section;

            $('#sectionname').text(_sectionname);
            $('#modal_confirmation').modal('show');
        });

        $('#btn_yes').click(function(){
            removeSection().done(function(response){
            ...
            if(validateRequiredFields($('#frm_section'))){
                if(_txnMode==="new"){
                    ...
                    createSection().done(function(response){
                        ...
                        $.unblockUI();
                        $('#modal_create_section').modal('toggle');
                    });
                    return;
                }
                if(_txnMode==="edit"){
                    //alert("update");
                    updateSection().done(function(response){
                        showNotification(response);
                        if(response.stat=="error"){
                            $.unblockUI();
                             }
    ...
    var updateSection=function(){
        ...
        console.log(_data);
        _data.push({name : "ref_section_id" ,value : _selectedID});
        ...
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"RefSection/transaction/update",
            "data":_data,
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
    ...
    var removeSection=function(){
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"RefSection/transaction/delete",
            "data":{ref_section_id : _selectedID},
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
